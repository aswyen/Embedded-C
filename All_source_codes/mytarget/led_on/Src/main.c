...
int main(void)
{
	...
	*pRCC_AHB1ENR |= RCC_AHB1ENR_GPIOA_EN; //Enable RCC Periph Clock to AHB1 -> GPIOA

	*pGPIOA_MODER &= ~(3 << (N64_LED2 * 2)); //Clear Mode before setting
	*pGPIOA_MODER |= (GPIO_MODE_OUT << (N64_LED2 * 2)); //Enable PA4, MODE Field is 2 bits wide

	*pGPIOA_ODR   |= (1 << N64_LED2); // Set PA4 High - LED2 ON
